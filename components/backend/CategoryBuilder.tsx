...
const CategoryBuilder: React.FC<CategoryBuilderProps> = ({ initialData, onSave, onCancel }) => {
  ...
  const handleSave = () => {
    ...
    const newCategory: Category = {
      id: initialData ? initialData.id : `cat_${Date.now()}`,
      name: formData.name,
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-2">
                {initialData ? `Modify details for category "${initialData.name}".` : 'Create a new product category for your store.'}
            </p>
          </div>
          <button 
            onClick={onCancel}
