...
interface AuthFormProps {
  type: 'LOGIN' | 'SIGNUP';
  onSuccess: (email?: string) => void; // Updated signature
...
const AuthForm: React.FC<AuthFormProps> = ({ type, onSuccess, onSwitchMode }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (type === 'LOGIN') {
           ...
           onSuccess(email); // Pass email to handle redirects
      } else {
      ...
      // Use Firebase/Supabase error code if available
      const errorCode = err.code || '';
      const errorMessage = err.message || '';

      if (errorCode === 'auth/invalid-email' || errorMessage.includes('auth/invalid-email') || errorMessage.includes('invalid_email')) {
        msg = 'Invalid email address.';
      } else if (errorCode === 'auth/email-already-in-use' || errorMessage.includes('already registered')) {
  ...
  const handleResetSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        console.error(err);
        // Fallback simulation is handled in AuthContext now, so error here means critical failure
    ...
    return (
      <div className="w-full max-w-md bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 animate-in fade-in zoom-in duration-300 text-center">
        ...
        <button
          ...
          className="w-full py-3 px-4 rounded-lg shadow-sm text-sm font-medium text-white bg-amber-600 hover:bg-amber-700 dark:bg-amber-50 dark:hover:bg-amber-600 transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500"
        >
          Return to Login
        </button>
      </div>
                ...
                <>
                    ...
                    <form onSubmit={handleResetSubmit} className="space-y-4">
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full flex justify-center items-center gap-2 py-3 px-4 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-amber-600 hover:bg-amber-700 dark:bg-amber-50 dark:hover:bg-amber-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                        >
                            {loading ? (
